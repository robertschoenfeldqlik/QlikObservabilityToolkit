...
| Layer | What it does |
|---|---|
| **MCP server** ([src/](src/)) | Auto-generates read-only **Talend** observability tools from OpenAPI specs (the observability preset exposes 8 tools), **plus a curated [Qlik Cloud](docs/observability-capabilities.md) observability tool family** (`qlik_observability__*` — apps, reloads, audits, quotas, spaces, users, reload-tasks). Every tool routes by `tenant` across multiple Talend **and** Qlik tenants. Ships two Talend presets — `observability` (default) and `logging` — widen with `TMC_APIS`. Includes a `tmc_list_environments` meta-tool so the model can discover configured tenants. See **[Observability capabilities](docs/observability-capabilities.md)**. |
